...
Num_Stats <- c("16 States","17 States","18 States")
Log_Likelihood_Test <- c(normed_16states_loglik_test,normed_17states_loglik_test,normed_18states_loglik_test)
Log_Likelihood_Train <- c(normed_16states_loglik_train,normed_17states_loglik_train,normed_18states_loglik_train)
Log_Likelihood_Diff <- c(log_lik_diff_16states,log_lik_diff_17states,log_lik_diff_18states)
...
log_like_df_test <- log_like_df_test %>% mutate("Log-Likelihood" = abs(Log_Likelihood_Test)) %>% mutate("Dataset" = "Test")
log_like_df_train <- log_like_df_train %>% mutate("Log-Likelihood" = abs(Log_Likelihood_Train)) %>% mutate("Dataset" = "Train")
log_like_df <- rbind(log_like_df_test,log_like_df_train)

p <- log_like_df %>% ggplot(mapping = aes(x=Num_Stats,y=`Log-Likelihood`,fill = Dataset))
p + geom_bar(position = "dodge",stat = "identity") + ylim(0,2) + labs(y = "|Log-Likelihood|",fill = "Number of States",x = "Number of States",title = "Test and Train Log-Likelihood per Number of States")
p2 <- log_like_df_diff %>% ggplot(mapping = aes(x=Num_Stats,y=Log_Likelihood_Diff))
p2 + geom_bar(stat = "identity",fill = "blue") + ylim(0,0.3) + labs(x = "Number of States",title = "Difference in Log-Likelihood Between Test and Train", y = "|Log-Likelihood|")
...
Anoma_log_lik <- c(log_lik_anomalous1,log_lik_anomalous2,log_lik_anomalous3)
Anoma_norm_log_lik <- c(normed_loglik_anomalous1,normed_loglik_anomalous2,normed_loglik_anomalous3)

Anoma_log_df <- data.frame(Anoma_list) %>% mutate(log_lik = Anoma_log_lik,type = "")
Anoma_norm_log_df <- data.frame(Anoma_list) %>% mutate(log_lik = Anoma_norm_log_lik,type = "")

p3 <- Anoma_norm_log_df %>% ggplot(mapping = aes(x = type,y = abs(log_lik),fill=Anoma_list))
p3 + geom_bar(position = "dodge",stat = "identity") + labs(x = "",fill = "Anamalous Dataset",y = "|Log-Likelihood|",title = "Normalized Log-Likelihood of Anamalous Datasets")
